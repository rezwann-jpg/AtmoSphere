...
void OpenWeatherParser::parseWeatherData(const nlohmann::json& jsonObj, DetailedWeather& weather) {
    try {
        ...
        if (jsonObj.contains("visibility") && jsonObj["visibility"].is_number_integer()) 
            weather.setVisibility(jsonObj["visibility"].get<int>());
        ...
        if (jsonObj.contains("weather") && jsonObj["weather"].is_array() && !jsonObj["weather"].empty()) {
            const auto& weatherArray = jsonObj["weather"][0];
            if (weatherArray.contains("main")) 
                weather.setWeatherMain(weatherArray["main"].get<std::string>());
            if (weatherArray.contains("description")) 
                weather.setWeatherDescription(weatherArray["description"].get<std::string>());
        }
    } catch (const std::exception& e) {
        logError("Error parsing weather data: " + std::string(e.what()));
    }
